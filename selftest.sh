#!/bin/bash
# End-to-end self test. Run it with the stack up.
#
...
. "$(cd "$(dirname "$0")" && pwd)/setenv.sh" >/dev/null 2>&1

if ! pgrep -f "procname idb1" >/dev/null 2>&1; then
  echo "ERROR: the stack is not running - start it with torq.sh start all" >&2
  exit 1
fi
